"""
Visualization script for MI_diff vs Generalization Gap analysis.

Creates scatter plots showing the relationship between generalization gap
and MI difference (MI_full - mean(MI_masked)) for different training ablations.
"""
...
def plot_mi_diff_vs_gen_gap(results, architecture: str, output_dir: Path):
    ...
    output_path = output_dir / f"{architecture}_mi_diff_vs_gen_gap.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {output_path}")

    # Also save as PDF for publications
    output_path_pdf = output_dir / f"{architecture}_mi_diff_vs_gen_gap.pdf"
...
def main():
    parser = argparse.ArgumentParser(
        description='Plot MI_diff vs Generalization Gap scatter plot'
    )
